...
function DashboardPage() {
  ...
  const generateMemberInsights = async () => {
    try {
      ...
      const membersResponse = await axios.get('fitness-frontend-0ri3.onrender.com/api/members/', {
      ...
      try {
        const attendanceResponse = await axios.get('fitness-frontend-0ri3.onrender.com/api/attendance/history/?today_only=true', {
          headers: { 'Authorization': `Bearer ${token}` }
        });
  ...
  return (
    <div className="p-6 bg-gray-200 min-h-screen">
      ...
      {/* Add the Last Member Credentials component here */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-white p-6 rounded-xl shadow-md hover:shadow-lg transition-shadow transform hover:-translate-y-1 duration-200">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-sm font-medium text-gray-400 uppercase tracking-wider mb-1">Total Revenue</p>
              {loading ? (
                <div className="animate-pulse h-8 bg-gray-200 rounded w-32 mb-1"></div>
              ) : (
                <p className="text-2xl font-bold text-gray-900">{formatAfn(stats.totalRevenue)}</p>
              )}
        ...
        <div className="bg-white p-6 rounded-xl shadow-md hover:shadow-lg transition-shadow transform hover:-translate-y-1 duration-200">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-sm font-medium text-gray-400 uppercase tracking-wider mb-1">Active Members</p>
              {loading ? (
                <div className="animate-pulse h-8 bg-gray-200 rounded w-16 mb-1"></div>
              ) : (
                <p className="text-2xl font-bold text-gray-900">{stats.activeMembers}</p>
              )}
              <p className="text-gray-500 text-sm mt-2">Total registered athletes</p>
            </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-md hover:shadow-lg transition-shadow transform hover:-translate-y-1 duration-200">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-sm font-medium text-gray-400 uppercase tracking-wider mb-1">Present Members</p>
              {loading ? (
                <div className="animate-pulse h-8 bg-gray-200 rounded w-16 mb-1"></div>
              ) : (
                <p className="text-2xl font-bold text-gray-900">{stats.presentMembers}</p>
              )}
        ...
        <div className="bg-white p-6 rounded-xl shadow-md hover:shadow-lg transition-shadow transform hover:-translate-y-1 duration-200">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-sm font-medium text-gray-400 uppercase tracking-wider mb-1">Total Check-ins</p>
              {loading ? (
                <div className="animate-pulse h-8 bg-gray-200 rounded w-16 mb-1"></div>
              ) : (
                <p className="text-2xl font-bold text-gray-900">{stats.totalAttendance}</p>
              )}
              <p className="text-gray-500 text-sm mt-2">Today's total attendance</p>
            </div>
      ...
      <div className="bg-white rounded-xl shadow-md p-6 mb-8">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-gray-800">Top Active Members</h2>
          <Link to="/admin/members" className="text-blue-500 hover:text-blue-700 flex items-center">
            <span>View All</span>
              ...
              {loading ? (
                ...
                recentMembers.map((member, index) => (
                  <tr key={member.id || member.athlete_id || index}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getMembershipStatusStyle(member.status)}`}>
      ...
      <div className="bg-white rounded-xl shadow-md p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Link to="/admin/register-member" className="flex items-center justify-center p-4 rounded-lg bg-blue-50 hover:bg-blue-100 transition-colors duration-200">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 mr-2 text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z" />
            </svg>
            <span>Register Member</span>
          </Link>
          <Link to="/admin/attendance" className="flex items-center justify-center p-4 rounded-lg bg-green-50 hover:bg-green-100 transition-colors duration-200">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 mr-2 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
            </svg>
            <span>Attendance Check</span>
          </Link>
          <Link to="/admin/trainings" className="flex items-center justify-center p-4 rounded-lg bg-purple-50 hover:bg-purple-100 transition-colors duration-200">
