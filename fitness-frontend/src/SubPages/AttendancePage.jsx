...
const AttendancePage = () => {
  ...
  const fetchAllMembersAttendanceCounts = async () => {
    ...
    try {
      const counts = {};
      // Option 1: If your backend supports a summary endpoint, use it!
      // Option 2: Otherwise, fetch for each member (not efficient for large lists)
      await Promise.all(
        members.map(async (member) => {
          try {
            const res = await axios.get(`${API_BASE_URL}/api/attendance/history/?member_id=${getMemberId(member)}`);
            const records = Array.isArray(res.data) ? res.data : res.data.attendance || [];
            counts[getMemberId(member)] = records.length;
          } catch {
            counts[getMemberId(member)] = 0;
          }
        })
      );
  ...
  const handleBiometricCheckIn = async () => {
    ...
    try {
      const res = await axios.post(`${API_BASE_URL}/api/biometrics/check-in/`, {
