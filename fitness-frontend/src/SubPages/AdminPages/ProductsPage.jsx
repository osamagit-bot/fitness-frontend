  ...
  return (
    <div id="products" className="p-2 md:p-4">
      ...
      <div className="bg-white p-4 rounded-lg shadow-md">
        <h2 className="text-lg font-semibold mb-4">Sold Products</h2>
        {soldLoading ? (
          <div className="flex justify-center py-8">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-green-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-green-600 uppercase">#</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-green-600 uppercase">Date</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-green-600 uppercase">Product Name</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-green-600 uppercase">Price (AFN)</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {soldProducts.map((sold, index) => (
                  <tr key={sold.id || index}>
                    <td className="px-6 py-4">{index + 1}</td>
                    <td className="px-6 py-4">{new Date(sold.date).toLocaleString()}</td>
                    <td className="px-6 py-4">{sold.product_name || (sold.product && sold.product.name)}</td>
                    <td className="px-6 py-4">{parseFloat(sold.total_price || sold.price).toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="text-center text-gray-500 py-6">No sold products found</div>
        )}
      </div>
    </div>
  );
