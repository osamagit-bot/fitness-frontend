...
function MemberCommunityPage() {
  ...
  const handleLike = async (postId) => {
    try {
      await api.post(`community/posts/${postId}/like/`, null, {
        headers: {
  ...
  const handleDeleteComment = async (postId, commentId) => {
    ...
    try {
      await api.post(
        "community/posts/comment/delete/",
        {
          commentId: commentId,
          postId: postId,
        },
  ...
  // Fixed formatDate function based on API response structure
  const formatDate = (dateString) => {
    if (!dateString) return "Unknown Date";

    try {
      // The API returns date in format: "2025-07-16T05:43:23.059237+00:00"
  ...
  const isOwner = (post, comment = null) => {
    if (comment) {
      console.log("Comment ownership check:", {
        ...
        isMatch: comment.author === memberName,
      });
      return comment.author === memberName;
    } else {
      console.log("Post ownership check:", {
        postAuthor: post.author,
        memberName: memberName,
        isMatch: post.author === memberName,
      });
      return post.author === memberName;
    }
  };
                  ...
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-gray-500 mb-2">
                        By {post.author || "Unknown"} on {formatDate(post.date)}
                      </div>
                    </div>

                    {/* ALWAYS SHOW BUTTONS FOR DEBUGGING - Remove isOwner check temporarily */}
                        ...
                        {(post.comments_list || []).map((comment) => (
                          ...
                          >
                            <div className="flex justify-between items-center mb-1">
                              ...
                              {/* ALWAYS SHOW COMMENT BUTTONS FOR DEBUGGING - Remove isOwner check temporarily */}
